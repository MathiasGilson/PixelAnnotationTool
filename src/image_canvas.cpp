...
ImageCanvas::ImageCanvas(MainWindow *ui) : QLabel(), _ui(ui), _pen_size(30) {
...
ImageCanvas::~ImageCanvas() { _scroll_parent->deleteLater(); }
...
void ImageCanvas::_initPixmap() {
    ...
    if (p != NULL) painter.drawPixmap(0, 0, *pixmap());
    painter.end();
    setPixmap(newPixmap);
}

void ImageCanvas::loadImage(const QString &filename) {
    if (!_image.isNull()) saveMask();
    ...
    _mask_file =
        file.dir().absolutePath() + "/" + file.baseName() + "_mask.png";
    _watershed_file = file.dir().absolutePath() + "/" + file.baseName() +
                      "_watershed_mask.png";
    ...
    if (QFile(_mask_file).exists()) {
        ...
        _ui->checkbox_manual_mask->setChecked(true);
    ...
    resize(_ui->scale * _image.size());
}

void ImageCanvas::saveMask() {
    if (isFullZero(_mask.id)) return;
    ...
    if (!_watershed.id.isNull()) {
        ...
        QFileInfo file(_img_file);
        QString color_file = file.dir().absolutePath() + "/" + file.baseName() +
                             "_color_mask.png";
...
void ImageCanvas::paintEvent(QPaintEvent *event) {
    ...
    QSize size = _ui->scale * _image.size();
    if (size != _image.size()) {
    ...
    painter.setOpacity(_ui->alpha);

    if (!_mask.id.isNull() && _ui->checkbox_manual_mask->isChecked()) {
    ...
    if (_mouse_pos.x() > 10 && _mouse_pos.y() > 10 &&
        ...
        painter.drawEllipse(_mouse_pos.x() / _ui->scale - _pen_size / 2,
                            _mouse_pos.y() / _ui->scale - _pen_size / 2,
                            _pen_size, _pen_size);
        painter.end();
    }
}
...
void ImageCanvas::mouseMoveEvent(QMouseEvent *e) {
    ...
    if (_button_is_pressed) _drawFillCircle(e);

    update();
}

void ImageCanvas::setSizePen(int pen_size) { _pen_size = pen_size; }
...
void ImageCanvas::mouseReleaseEvent(QMouseEvent *e) {
    ...
    if (e->button() == Qt::RightButton) {  // selection of label
        QColor color = _mask.id.pixel(_mouse_pos / _ui->scale);
        const LabelInfo *label = _ui->id_labels[color.red()];

        if (!_watershed.id.isNull() &&
            _ui->checkbox_watershed_mask->isChecked()) {
            QColor color = QColor(_watershed.id.pixel(_mouse_pos / _ui->scale));
            QMap<int, const LabelInfo *>::const_iterator it =
                _ui->id_labels.find(color.red());
...
void ImageCanvas::_drawFillCircle(QMouseEvent *e) {
    if (_pen_size > 0) {
        int x = e->x() / _ui->scale - _pen_size / 2;
        int y = e->y() / _ui->scale - _pen_size / 2;
        _mask.drawFillCircle(x, y, _pen_size, _color);
    } else {
        int x = (e->x() + 0.5) / _ui->scale;
        int y = (e->y() + 0.5) / _ui->scale;
...
void ImageCanvas::wheelEvent(QWheelEvent *event) {
    ...
    if (Qt::ShiftModifier == event->modifiers()) {
        ...
        int value = _ui->spinbox_pen_size->value() +
                    delta * _ui->spinbox_pen_size->singleStep();
        ...
        _scroll_parent->verticalScrollBar()->setEnabled(false);
        double value = _ui->spinbox_scale->value() +
                       delta * _ui->spinbox_scale->singleStep();
...
void ImageCanvas::setMask(const ImageMask &mask) { _mask = mask; }
