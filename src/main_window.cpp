...
MainWindow::MainWindow(QWidget *parent, Qt::WindowFlags flags)
    ...
    setWindowTitle(QApplication::translate(
        "MainWindow", "PixelAnnotationTool " PIXEL_ANNOTATION_TOOL_GIT_TAG,
        Q_NULLPTR));
    ...
    connect(actionOpen_config_file, SIGNAL(triggered()), this,
            SLOT(loadConfigFile()));
    connect(actionSave_config_file, SIGNAL(triggered()), this,
            SLOT(saveConfigFile()));
    connect(close_tab_action, SIGNAL(triggered()), this,
            SLOT(closeCurrentTab()));
    connect(tabWidget, SIGNAL(tabCloseRequested(int)), this,
            SLOT(closeTab(int)));
    connect(tabWidget, SIGNAL(currentChanged(int)), this,
            SLOT(updateConnect(int)));
    connect(tree_widget_img, SIGNAL(itemClicked(QTreeWidgetItem *, int)), this,
            SLOT(treeWidgetClicked()));
    ...
    connect(list_label, SIGNAL(itemDoubleClicked(QListWidgetItem *)), this,
            SLOT(changeColor(QListWidgetItem *)));
...
void MainWindow::closeTab(int index) {
    ...
    if (ic == NULL) throw std::logic_error("error index");

    if (ic->isNotSaved()) {
        QMessageBox::StandardButton reply =
            QMessageBox::question(this, "Current image is not saved",
                                  "You will close the current image, Would you "
                                  "like saved image before ?",
...
void MainWindow::changeColor(QListWidgetItem *item) {
    LabelWidget *widget =
        static_cast<LabelWidget *>(list_label->itemWidget(item));
...
void MainWindow::changeLabel(QListWidgetItem *current,
    ...
    if (previous == NULL) {
        for (int i = 0; i < list_label->count(); i++) {
            LabelWidget *label = static_cast<LabelWidget *>(
                list_label->itemWidget(list_label->item(i)));
    ...
    sstr << "label=[" << key << "] id=[" << labels[key].id << "] categorie=["
         << labels[key].categorie << "] color=[" << labels[key].color.name()
         << "]";
...
void MainWindow::updateConnect(const ImageCanvas *ic) {
    if (ic == NULL) return;
    connect(spinbox_scale, SIGNAL(valueChanged(double)), ic,
            SLOT(scaleChanged(double)));
    connect(spinbox_alpha, SIGNAL(valueChanged(double)), ic,
            SLOT(alphaChanged(double)));
    connect(spinbox_pen_size, SIGNAL(valueChanged(int)), ic,
            SLOT(setSizePen(int)));
    connect(checkbox_watershed_mask, SIGNAL(clicked()), ic, SLOT(update()));
    connect(checkbox_manual_mask, SIGNAL(clicked()), ic, SLOT(update()));
...
void MainWindow::allDisconnnect(const ImageCanvas *ic) {
    if (ic == NULL) return;
    disconnect(spinbox_scale, SIGNAL(valueChanged(double)), ic,
               SLOT(scaleChanged(double)));
    disconnect(spinbox_alpha, SIGNAL(valueChanged(double)), ic,
               SLOT(alphaChanged(double)));
    disconnect(spinbox_pen_size, SIGNAL(valueChanged(int)), ic,
               SLOT(setSizePen(int)));
    disconnect(checkbox_watershed_mask, SIGNAL(clicked()), ic, SLOT(update()));
    disconnect(checkbox_manual_mask, SIGNAL(clicked()), ic, SLOT(update()));
...
ImageCanvas *MainWindow::getImageCanvas(int index) {
    QScrollArea *scroll_area =
        static_cast<QScrollArea *>(tabWidget->widget(index));
...
void MainWindow::treeWidgetClicked() {
    ...
    QString iDir = currentDir();
    if (iFile.isEmpty() || iDir.isEmpty()) return;
    allDisconnnect(image_canvas);
...
void MainWindow::on_tree_widget_img_currentItemChanged(
...
void MainWindow::on_actionOpenDir_triggered() {
    ...
    QString openedDir = QFileDialog::getExistingDirectory(
        this, "Choose a directory to be read in", curr_open_dir);
    if (openedDir.isEmpty()) return;
    ...
    static QStringList ext_img = {"png", "jpg", "bmp", "pgm",  "jpeg", "jpe",
                                  "jp2", "pbm", "ppm", "tiff", "tif"};
    for (int i = 0; i < files.size(); i++) {
        ...
        if (files[i].toLower().indexOf("_mask.png") > -1) continue;
...
void MainWindow::saveConfigFile() {
    QString file = QFileDialog::getSaveFileName(
        this, tr("Save Config File"), QString(), tr("JSon file (*.json)"));
...
void MainWindow::loadConfigFile() {
    QString file = QFileDialog::getOpenFileName(
        this, tr("Open Config File"), QString(), tr("JSon file (*.json)"));
